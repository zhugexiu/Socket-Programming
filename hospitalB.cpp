...
struct sockaddr_in serverAddrUDP; //hospitalB use to create UDP 
    ...
    while(true) {
        ...
        if(choice == "Update"){
            ...
            if(availability < 0 || availability > 1) {
                cout << "Hospital B has been assigned to a client, occupation is updated to " << occupancy << " ,availability is updated to None" << endl;
            }else{
                cout << "Hospital B has been assigned to a client, occupation is updated to " << occupancy << " ,availability is updated to ";
                cout << availability << endl;
            }
        }else{
            string position = clientPosition;
            cout << "Hospital B has received input from client at location " << position << endl;
            pos = clientPosition;
            ...
            if(p_of_client == p_of_hospital) {
                ...
                if(::sendto(sockfdUDP,output, sizeof(output), 0, (const struct sockaddr *) &clientAddrUDP,
                    ...
                    perror("Hospital B response failed");
                    exit(1);
                }
                cout << "Hospital B has the same location with client" << endl;
...
void readFile(){
   ...
   /**
    ...
    if(map_index.count(pos) == 0){
        ...
        cout << "Hospital B does not have the location " << pos <<" in map" << endl;
        ...
        **/
        ...
        if(availability >= 0 && availability <= 1) {
            cout << "Hospital B has capacity = " << capacity_int << " occupation = " << occupancy_int <<", availability = " << availability << endl;
            cout << "Hospital B has found the shortest path to client,distance = " << dis_res << endl;
            //score = 1/(d*(1.1-a))
            ...
            cout << "Hospital B has sent score = " << to_string(score) << " and distance = " << to_string(dis_res) << " to the Scheduler" << endl;

        }else{
            cout << "Hospital B has capacity = " << capacity_int << " occupation = " << occupancy_int <<", availability = " << "None" << endl;
            cout << "Hospital B has found the shortest path to client,distance = " << dis_res << endl;
            ...
            cout << "Hospital B has sent score = " << "None" << " and distance = " << to_string(dis_res) << "to the Scheduler" << endl;
        }
    }
...
void createUDPSocket(){
    ...
    printf("Hospital B is up and running using UDP on port %i. \nHospital B has total capacity %s and initial occupancy %s \n", UDP_PORT,capacity.c_str(), occupancy.c_str());
...
void sendInfo(){
    ...
    if (::recvfrom(sockfdUDP, hospitalInputFeedback, BUFLENGTH, 0, (struct sockaddr *) &clientAddrUDP,
        ...
        perror("HospitalB receive failed");
        exit(1);
    }

    cout << "Hospital B has sent the information to Scheduler" << endl;
    cout << endl;
}
